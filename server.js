...
function VerifyTokenMiddleware(req, res, next) {
  if (req.headers.authorization !== undefined) {
    ...
    res.status(401).send({ status: 401, message: "Valid JWT token is required" });
  }
}
...
server.get('/auth/favorites', VerifyTokenMiddleware, (req, res) => {
  userdb = JSON.parse(fs.readFileSync('./users.json', 'UTF-8'))
  res.json(userdb.favorites);
});

server.post('/auth/favorites', VerifyTokenMiddleware, (req, res) => {
  // Get current users data
  var data = JSON.parse(fs.readFileSync('./users.json', 'UTF-8'))

  // Get the id of last user
  var last_item_id;
  if (data.favorites[data.favorites.length - 1] === undefined) {
    last_item_id = 0;
  } else {
    last_item_id = data.favorites[data.favorites.length - 1].id;
  }

  //Add new user
  data.favorites.push({ id: last_item_id + 1, ...req.body });
